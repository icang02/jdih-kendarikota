...
class PeraturanController extends Controller
{
  ...
  public function actionCreate2()
  {
    ...
    if ($model->load(Yii::$app->request->post()) && $lampiran->load(Yii::$app->request->post()) && $teu->load(Yii::$app->request->post())) {
      ...
      if (!empty($abstrak)) {
        $model->abstrak = $this->gen_filename($abstrak->name);
      ...
      if (!empty($dokumen_lampiran)) {
        $lampiran->dokumen_lampiran = $this->gen_filename($dokumen_lampiran->name);
  ...
  public function actionCreate()
  {
    ...
    if ($model->load(Yii::$app->request->post()) && $lampiran->load(Yii::$app->request->post())) {
      ...
      $lampiran->url_lampiran = !empty($dokumen_lampiran) ?
        Yii::getAlias('@imageurl') . '/common/dokumen/' . $lampiran->dokumen_lampiran :
        null;
      $lampiran->save();
  ...
  public function actionUpdate($id)
  {
    ...
    if ($model->load(Yii::$app->request->post())) {
      ...
      if (!empty($abstrak)) {
        ...
        $deleteAbstrak = Yii::getAlias("@common/dokumen/$oldAbstrak");
        if (is_file($deleteAbstrak))
          unlink($deleteAbstrak);
      } else {
        $model->abstrak = $oldAbstrak;
      }
  ...
  public function actionDelete($id)
  {
    try {
      ...
      if ($model->delete()) {
        // Delete abstrak
        $deleteAbstrak = Yii::getAlias("@common/dokumen/$model->abstrak");
        if (is_file($deleteAbstrak))
          unlink($deleteAbstrak);

        // Delete dokumen lampiran
        $deleteDokumenLampiran = Yii::getAlias("@common/dokumen/" . $model->dokumen->dokumen_lampiran);
        if (is_file($deleteDokumenLampiran))
          unlink($deleteDokumenLampiran);

        // Delete hasil uji materi
        $hasilUjiMateri = HasilUjiMateri::find()->where(['id_dokumen' => $id])->all();
        foreach ($hasilUjiMateri as $ujiMateri) {
          $deleteHasilUjiMateri = Yii::getAlias("@common/dokumen/" . $ujiMateri->hasil_uji_materi);
          if (is_file($deleteHasilUjiMateri))
            unlink($deleteHasilUjiMateri);
          $ujiMateri->delete(); //
        }
      }
  ...
  public function actionUbahLampiran($id)
  {
    ...
    if ($model->load(Yii::$app->request->post())) {
      $dokumen_lampiran = UploadedFile::getInstance($model, 'dokumen_lampiran');
      if (!empty($dokumen_lampiran)) {
        $model->dokumen_lampiran =  $this->gen_filename($dokumen_lampiran->name);
        $path = Yii::getAlias('@common') . '/dokumen/' . $model->dokumen_lampiran;
        $dokumen_lampiran->saveAs($path);

        $model->url_lampiran = !empty($dokumen_lampiran) ?
          Yii::getAlias('@imageurl') . '/common/dokumen/' . $model->dokumen_lampiran : null;

        // Delete old lampiran
        $deleteDokumen = Yii::getAlias("@common/dokumen/$old");
  ...
  public function actionTambahHasilUjiMateri($id)
  {
    ...
    if ($model->load(Yii::$app->request->post())) {
      ...
      if (!empty($hasil_uji_materi)) {
        $model->hasil_uji_materi = $this->gen_filename($hasil_uji_materi->name);
  ...
  public function actionHapusHasilUjiMateri($id)
  {
    ...
    if ($model->delete()) {
      $deleteDokumen = Yii::getAlias("@common/dokumen/$model->hasil_uji_materi");
      if (is_file($deleteDokumen))
        unlink($deleteDokumen);
    }
