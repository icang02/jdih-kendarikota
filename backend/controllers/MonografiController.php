...
class MonografiController extends Controller
{
  ...
  public function actionCreate()
  {
    ...
    if ($model->load(Yii::$app->request->post())) {
      ...
      if (!empty($cover)) {
        $model->gambar_sampul = $this->gen_filename($cover->name);
  ...
  public function actionUpdate($id)
  {
    ...
    if ($model->load(Yii::$app->request->post())) {
      ...
      if (!empty($abstrak)) {
        ...
        // Delete abstrak old
        $deleteAbstrak = Yii::getAlias("@common/dokumen/$old_abstrak");
      ...
      if (!empty($cover)) {
        ...
        // Delete cover old
        $deleteCover = Yii::getAlias("@common/dokumen/$old_sampul");
        if (is_file($deleteCover))
          unlink($deleteCover);
      } else {
  ...
  public function actionDelete($id)
  {
    try {
      ...
      if ($model->delete()) {
        // Delete cover
        $deleteCover = Yii::getAlias("@common/dokumen/$model->gambar_sampul");
        if (is_file($deleteCover))
          unlink($deleteCover);
        ...
        // Delete lampiran
        $lampiranAll = DataLampiran::findAll(['id_dokumen' => $id]);
        foreach ($lampiranAll as $lampiran) {
          $deleteDokumen = Yii::getAlias("@common/dokumen/{$lampiran->dokumen_lampiran}");
  ...
  public function actionHapusLampiran($id)
  {
    ...
    // Hapus model dari database
    if ($model->delete()) {
      $deleteDokumen = Yii::getAlias("@common/dokumen/$model->dokumen_lampiran");
      if (is_file($deleteDokumen))
        unlink($deleteDokumen);
    }
